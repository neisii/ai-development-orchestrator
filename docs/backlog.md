...
| 사용자 프로젝트에서 Claude Code 세션과 직접 검토할 때 | **`SendMessage`+`PreToolUse` Hook으로 거버넌스를 조화시키는 아이디어** — [concept-governance-layer.md §3~4](concept-governance-layer.md#3-sendmessage와-거버넌스를-조화시킨다면). Hook이 teammate 간 `SendMessage` 호출을 예외 없이 가로챌 수 있는지가 미확인 전제라, 이 저장소에서 먼저 손대지 않고 검토만 남겨둠 |
