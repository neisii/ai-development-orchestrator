...
- ~~위 기능들의 정확한 도입 버전 (공식 문서에 명시 없음, 필요시 Anthropic 문의)~~ → 조사 안 하기로 함(2026-08-27). 동작 자체는 이미 실측으로 확정돼 있고 어느 버전부터인지는 코드에 영향이 없는 순수 히스토리 정보라, Anthropic에 직접 문의하는 수고 대비 얻는 게 없다고 판단.
- ~~일부 환경에서는 Bash 도구 호출이 즉시 실행되지 않고 자체 백그라운드 Task로 위임되어(`task_started`/`task_notification` 이벤트) 예상보다 훨씬 빨리 끝나는 것이 관찰됐다. 오케스트레이터가 "도구 실행 중 여부"를 판단할 때 이 가능성을 감안해야 할 수 있다.~~ → 격리 환경에서 일부러 느린 Bash 명령(`sleep 10 && echo ...`)을 실행시켜 `TOOL_PRE`~`TOOL_POST` 간격을 측정(2026-08-26). 실측 10906ms로 실제 소요 시간(~10000ms)과 거의 정확히 일치 — 내부적으로 Task 위임이 일어나더라도 hook 기반 `TOOL_PRE`/`TOOL_POST` 타이밍은 실제 실행 구간을 정확히 감싼다. 오케스트레이터가 이 타이밍만으로 "실행 중 여부"를 판단하는 데는 문제가 없음을 확인했으므로, 일반 동작인지 환경 특이 동작인지는 더 조사하지 않기로 함(오케스트레이터에 미치는 영향이 없어 우선순위 없음).
